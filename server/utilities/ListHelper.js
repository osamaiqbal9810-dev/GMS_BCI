...
export default class ListHelper {
    ...
    if (settings && settings != "") {
      ...
      if (settingsObj.hasOwnProperty("limit")) {
        ...
        if (Number.isNaN(parsedLimit)) {
          this.logger.warn(
            listName + ": limit parameter was not parsed, ignoring the limit"
          );
          console.log(
            listName + ": limit parameter was not parsed, ignoring the limit"
          );
    ...
    query.exec(async (err, data) => {
      ...
      if (data) {
        ...
        data.forEach(element => {
          ...
          descriptionValue = this.getValueNested(
            element._doc,
            fieldMap.description
          );
          optParam2Value = this.getValueNested(
            element._doc,
            fieldMap.optParam2
          );
  ...
  async addOrUpdate(item, callbacks) {
    ...
    else 
    {
      ...
      listModel.findByIdAndUpdate(item.code, itm, (err, result) => {
            if (err) {
