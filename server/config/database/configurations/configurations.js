...
export async function updateConfigurations(updateConfigs, appName)
{
    ...
    for(let config of configs)
    {
        ...
        {
            let cfg = appLookupConfigurations.find(c=>{return c.code===config.code;});
            if(cfg && cfg.opt1)
            {
...
async function checkAndUpdateConfigDescription(configlist)
{
    ...
    {   let configsToMod = await ApplicationLookupsModel.find({listName:"config", code:{$in:configlist}});
        if(configsToMod && configsToMod.length)
        {
            ...
            {
                let defCfg = appLookupConfigurations.find(c=>{return c.code===cfg.code;});
...
{
    let hiddenConfigs = appLookupConfigurations.filter(c=>{return c.opt1.hide;});
    if(appName==='SCIM')
    {
        const filter = {
    ...
    {
        let hiddenconfigcodes = hiddenConfigs.map(hc=>{return hc.code;});
        let configsToHide = await ApplicationLookupsModel.find({listName:"config", code:{$in:hiddenconfigcodes}, $or:[{"opt1.hide":{$exists: false}}, {"opt1.hide": false }]});
...
//
async function updateIndividualConfigurations(list)
{
    if(list && list.length)
    {
        for(let l2u of list)
        {
            ...
            let item2compare=appLookupConfigurations.find(a=>{return a.listName===l2u.listName && a.code===l2u.code;});
