...
import { railRoadLocationsTemplate } from "../../template/railRoadLocationsTemplate.js";
import { permissionsToAdd, inspectorPermissions } from "./permissionRoles.js";

import { LampAttributes } from "../../template/assetTypeAttributes.js";

import { addIfNotExist, renameField, update, UpdateOrAddIfNotExist } from "./dbFunctions/dbHelperMethods.js";
import { dbAnalyzerMethod } from "../../dbAnalyzer/dbAnalyzerMethod.js";
import { defectCodes } from "./defectCodes.js";
import { trackInspForm, inspectionInstructions } from "../../template/assetTypeAttributes.js";
import { addConfigurations, updateConfigurations } from "./configurations/configurations.js";
import { createApplicationLookups, updateApplicationLookups, deleteApplicationLookups } from "./configurations/applicationlookupslist.js";
import { createSupportedParams } from "./configurations/supportedParamsList.js";
import { createSupportedDevices } from "./configurations/supportedDevicesList.js";
import { setAssetTypesToInspectable, setAssetTypesDisplayNameProperty, removeRailsAssetType } from "./dbFunctions/assetTypesChangesOct2020.js";
import { removeAssetTypesForIOC, addIOCMissingAssetTypes } from "./dbFunctions/IOCMissingAssetTypes.js";
import { guid } from "../../utilities/UUID.js";
import {createEvents} from "./configurations/eventsList.js";


//import {addFerromexAssets} from "../../utilities/FerromexAssets.js";
//import { addLampAssetsForNHSLT1, trackAssetPopulateNSHL, lampNHSLAssets } from "../../utilities/nhslTracksAssetPopulate.js";
...
async function createDatabase() {
  ...
  await createApplicationLookups();
  await createSupportedParams();
  await createSupportedDevices();
  ...
  await addConfigurations(false);

  // Operational Contract Specific Data
  //await createAssetTypes();

  await createAssets("railroad");
  ...
  await setAssignedLocationToUsers(true);
  await updateOldDatabase(false);
  await updatePermissionsOfUserGroups(false);
  await updateConfigurations(false, "SCIM"); // call this with true to hide required configs. second param: App name for which configs needs to be set, SCIM for scim app; TIMPS for timps app
  await updateInspectionAndTemplates(false);
  ...
  await setAssetTypesToInspectable(false, "SCIM"); // TIMPS or SCIM as second argument
  await setAssetTypesDisplayNameProperty(false);
  await removeRailsAssetType(false);
  ...
  await updateApplicationLookupsPrev(false);

  await removeAssetTypesForIOC(false);
  await addIOCMissingAssetTypes(false);
...
async function createUserGroups() {
  ...
  for (const grp of groupsToAdd) {
    await UpdateOrAddIfNotExist(UserGroup, grp, grp, {
...
async function createAssets() {
  ...
  await addIfNotExist(AssetsModel, usersToAdd);
}
...
async function setAssignedLocationToUsers(execute) {
  if (!execute) return;
  let location = await AssetModel.findOne({
    parentAsset: null,
    assetType: "Company",
  });
  if (location) {
...
async function updateOldDatabase(execute) {
  ...
  let assets2Mod = await AssetModel.find({
...
async function updateApplicationLookupsPrev(execute) {
  if (execute) {
...
async function updateInspectionAndTemplates() {
  try {
    ...
    for (let temp of templates) {
      ...
      let inspections = await JourneyPlanModel.find({
  ...
  if (execute) {
    ...
    for (let loc of LocationTypes) {
      if (loc.assetType !== "Company") {
        ...
        let locationAssets = await AssetModel.find({
...
async function updateAssetTypesMakeTrackLowestInspectable(execute) {
  if (execute) {
    let trackAssetType = await AssetsTypeModel.findOne({
      assetType: "track",
    }).exec();
    if (trackAssetType) {
      for (let allowedTrackItem of trackAssetType.allowedAssetTypes) {
        let aItem = await AssetsTypeModel.findOne({
