...
exports.genAts = async function (req, res, next) {
  let AssetsService = ServiceLocator.resolve("AssetsService");
  //let resultObj = await AssetsService.getAllAssetsLamp(req.user);
  let resultObj = await AssetsService.getDevices();
  if (resultObj.errorVal) {
    return res.send(resultObj.errorVal);
  }
  res.status(200);
...
exports.getAts = async function (req, res, next) {
  let AssetsService = ServiceLocator.resolve("AssetsService");
  //let resultObj = await AssetsService.getAllAssetsLamp(req.user);
  let resultObj = await AssetsService.getAtsdevices(req.user, req.query);
  if (resultObj.errorVal) {
    return res.send(resultObj.errorVal);
  }
  res.status(200);
...
exports.assignDevice = async function (req, res, next) {
  let AssetsService = ServiceLocator.resolve("AssetsService");
  let resultObj; //= await AssetsService.createAssetsLamp(req.body.asset);
  resultObj = await AssetsService.assignDevice(req.body);
  if (resultObj.errorVal) {
    return res.send(resultObj.errorVal);
  }
  res.status(200);
...
exports.findDevicesPerLocation = async function (req, res) {

  let AssetsService = ServiceLocator.resolve("AssetsService");
  let resultObj = await AssetsService.findDevicesAtLocation(req.params.id);
  if (resultObj.errorVal) {
    return res.send(resultObj.errorVal);
  }
  res.status(200);
...
exports.getFloorAndSensors = async function (req, res) {
  //console.log(req.body);
  let AssetsService = ServiceLocator.resolve("AssetsService");
  let resultObj = await AssetsService.getFloorAndSensors(req.params.id);
  if (resultObj.errorVal) {
    return res.send(resultObj.errorVal);
  }
  res.status(200);
...
exports.updateSettings = async function (req, res, next) {
  ...
  let assetService = ServiceLocator.resolve("AssetsService");
  let resultObj = await assetService.updateSettings(req.params.id ,req.body);
  res.status(resultObj.status);
  if (resultObj.value) res.json(resultObj.value);
  else res.json(resultObj.errorVal);
};
...
exports.getAssetTree = async function (req, res) {
  ...
  let resultObj = await AssetsTreeService.getTreeByUser(req.user.assignedLocation);
  res.status(resultObj.status);
  if (resultObj.value) {
    res.json(resultObj.value);
  } else {
