...
import _, { result } from "lodash.js";
class AssetsTreeService {
  ...
  findTreeNode(treeBranch, _id) {
    ...
    for (let key of aIds) {
      ...
      if (typeof treeBranch[key] === "object") {
        let foundObj = this.findTreeNode(treeBranch[key], _id);
        if (foundObj) {
          return treeBranch[key];
        }
      }
  ...
  async getTreeByUser(nodeId) {
    ...
    try {
      ...
      if (assetTree.assetsTreeObj) {
        let found = false;
        await this.recursivelyFindAssetId(nodeId, assetTree.assetsTreeObj, foundAssetInsideTree, found);
        resultObj.value = foundAssetInsideTree;
      }
    } catch (err) {
      resultObj = { errroVal: err, status: 500 };
      console.log("error in getTreeByUser : ", err);
    }
    return resultObj;
  }
