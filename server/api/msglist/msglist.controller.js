...
async function addorUpdate(req, res, next) {
  ...
  req.body.forEach(async element => {
    ...
    if (vr.valid === true) {
      ...
      await DOEventService.trigger(item.listName, "BeforeAddOrUpdate", {
        item: item,
        user: req.user
      });
      ...
      listHelper.addOrUpdate(item, {
        success: async r1 => {
          await DOEventService.trigger(item.listName, "AddOrUpdateSuccess", {
            item: prevItemCopy,
            user: req.user
          });
          itemCount++;
        ...
        fail: async r1 => {
          await DOEventService.trigger(item.listName, "AddOrUpdateFailed", {
            item: item,
            user: req.user
          });
          itemCount++;
