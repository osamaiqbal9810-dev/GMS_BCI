...
class WorkOrderService {
  ...
  async createNewMWONumber() {
    let utils = ServiceLocator.resolve("utils");
    let wo1 = await WorkOrderModel.find()
      .sort({ mwoNumber: -1 })
      .limit(1)
      .exec();
    ...
    try {
      ...
      if (woToUpdate) {
        if (workOrder.maintenanceRequests && Array.isArray(workOrder.maintenanceRequests)) {
          ...
          if (mrsToExclude.length > 0) await maintenanceService.setWorkOrderNumber(mrsToExclude, null);
        ...
        if ((woToUpdate.status === "New" || woToUpdate.status === "Planned") && workOrder.dueDate) {
          ...
          let maintenanceService = ServiceLocator.resolve("MaintenanceService");
          await maintenanceService.setMRsFields(woToUpdate.maintenanceRequests, { status: desiredStatus, dueDate: workOrder.dueDate });
