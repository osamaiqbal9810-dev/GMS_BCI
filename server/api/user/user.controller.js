...
import * as permitTypes from "../../config/permissions.js"; // End
...
exports.index = async function(req, res) {
  ...
  if ( _.find(req.user.userGroup.permissions, { resource: "USER", action: "view",}))
   {
    let criteria = {};
    let ids = [];
      ...
      function(token, user, done) {
        let mailOptions = {
          ...
          html:
           ...
           "<p>" + config.server_comm_protocol + "://" + config.ip + ":" + config.port +
            // "<p>https://" + config.ip + ":" + config.port +
            "/confirmreset/" +
            token +
            "</p>" +
           "<a href='" + config.server_comm_protocol + "://" + config.ip + ":" + config.port +
...
exports.destroy = function(req, res) {
  User.findOne({ _id: req.params.id }, async (err, user) => {
    ...
    if(wpts.value && wpts.value.length && wpts.value.length > 0 ) // do not delete if inspection(s) is/are assigned
    {
