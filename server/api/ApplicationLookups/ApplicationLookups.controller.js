...
exports.getRefreshRate = async function(req, res, next)
{
  let applicationLookupsService = ServiceLocator.resolve("ApplicationLookupsService");

  let resultObj = await applicationLookupsService.getRefreshRate();
  res.status(resultObj.status);
  
  if (resultObj.newRefreshTime) res.json(resultObj);
  else res.json(resultObj.errorVal);
}
