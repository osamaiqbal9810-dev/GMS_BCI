...
class MaintenanceService {
  ...
  async workOrderNumberCreator() {
    let utils = ServiceLocator.resolve("utils");
    let ment1 = await MaintenanceModel.find()
  ...
  async updateFromWeb(mntnce) {
    ...
    try {
      ...
      if (mntnceToUpdate) {
        ...
        if (mntnceToUpdate.status === "New" || mntnceToUpdate.status === "Planning" || mntnceToUpdate.status === "Planned"
            || mntnceToUpdate.status === "In Progress") {   // allow change of estimate for In Progress status maintenances
  ...
  async setWorkOrderNumber(maintenanceList, workOrderNumber) {
    ...
    try {
      if (maintenanceList && maintenanceList.length) {
        await MaintenanceModel.update({ mrNumber: { $in: maintenanceList } }, { workOrderNumber: workOrderNumber }, { multi: true });
      }
    } catch (err) {
  ...
  async setMRsFields(
    ...
    try {
      if (maintenanceList && maintenanceList.length) {
        await MaintenanceModel.update({ mrNumber: { $in: maintenanceList } }, valuesObj, { multi: true });
      }
    } catch (err) {
