...
exports.all = async function (req, res, next) {
  ...
  if (result.value && result.value.length) {
    for (let template of result.value) {
      ...
      let alertRules = await alertService.fetchAlertsByModelId(template._id);

      alertRules = alertRules.map((al) => {
        al = al.toObject();
...
exports.update = async function (req, res, next) {
  ...
  let workPlanTemplateService = ServiceLocator.resolve("WorkPlanTemplateService");
  let result = await workPlanTemplateService.createWorkplanTemplate(plan);
  res.status(result.status);
  if (result.errorVal && result.status == 500) {
    return res.send(result.errorVal);
  }
