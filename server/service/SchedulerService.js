...
export default class SchedulerService {
  getSchedules(template, startDate, executed_schedules, dateRange, workingDays, ignoreExecutionsMethod) {
    let config = {
  ...
  getScheduleOfFreq(template, executed_schedules, dateRange, workingDays, freqOption, config, startDate, ignoreExecutionsMethod) {
    ...
    if (startDate <= DATE_FILTER_TO) {
      let currentDatePoint = new Date(startDate);
      //   let prevDatePoint;
      ...
      let endOfTimePeriod = getEndTimePeriod(currentDatePoint, freqOption);
      ...
      for (let e = 0; e < lengthOfFilteredSchedules; e++) {
        ...
        while (filteredSchedules[e].date > endOfTimePeriod) {
          ...
          if (shedsInCurrentTimePeriod.length >= freqOption.freq) {
            ...
            endOfTimePeriod = getEndTimePeriod(nextTimePeriodStart, freqOption);
            currentDatePoint = nextTimePeriodStart;
      ...
      while (!afterEndOfRange) {
        ...
        if (shedsInCurrentTimePeriod.length < freqOption.freq) {
          ...
          if (compareTwoDates(expiryDateToCheck, moment(DATE_FILTER_TODAY).startOf("day"), "IB")) {
            ...
            let futureSchedule = foreCastedInspectionObjectGet(template, expiryDateToCheck, "Future Inspection");
...
function getEndTimePeriod(datePoint, freqOption) {
  let date = moment(moment(datePoint).add(freqOption.timeFrameNumber, freqOption.timeFrame)).subtract(1, "minutes");
  return date;
}
...
function updateNextScheduleDueToToday(
  ...
  if (foundToday) {
    ...
    let timeZone = moment(futureSchedule.date).tz();
    if (timeZone) {
      todayDueDate = new Date(moment().tz(timeZone));
    } else {
...
function calculateDueDate(schedule, currentTimePeriod, shedsInCurrentTimePeriod, templateSchedules, config, freqOption) {
  ...
  if (dateOfLastSchedule) {
    let daysToAdd = config.minDaysGapBetweenTimePeriodsInFutureSchedules && freqOption.minDays > 0 ? freqOption.minDays + 1 : 1;
    dateToDue = moment(dateOfLastSchedule).add(daysToAdd, "days");
  ...
  if (dateToDue) {
    ...
    schedule.dueDate = dateToDue && moment(dateToDue).format();
...
function nextMaxIntervalExpiryDate(shedsInCurrentTimePeriod, templateSchedules, currentTimePeriod, freqOption) {
  ...
  if (dateOfLastSchedule) {
    ...
    if (!ignoreCheckMaxIntervalWithinTimePeriod) {
      maxExpiryDate = moment(dateOfLastSchedule).add(freqOption.maxInterval, "days");
...
function getLastDateOfScheduleWithTimePeriodChange(shedsInCurrentTimePeriod, templateSchedules, lastTimePeriodScheduleCondition) {
  let result, timePeriodChange, dateOfLastSchedule;
